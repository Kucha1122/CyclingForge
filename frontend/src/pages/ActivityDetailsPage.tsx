...
export default function ActivityDetailsPage() {
  ...
  const processStreams = (json: string) => {
    try {
      ...
      for (let i = 0; i < length; i++) {
        data.push({
          distance: distanceStream?.data[i] ? (distanceStream.data[i] / 1000).toFixed(2) : 0, // km
          ...
          speed: velocityStream?.data[i] ? (velocityStream.data[i] * 3.6).toFixed(1) : null, // km/h
